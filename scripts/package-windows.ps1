...
if ($Arch -eq "arm64") {
    & $cmake.Source --install $buildDir --config Release --prefix $deployDir
    if ($LASTEXITCODE -ne 0) { throw "CMake Qt deployment failed." }
} else {
...
if (-not (Test-Path (Join-Path $deployDir "HexImg.exe"))) {
    throw "Deployed HexImg.exe was not found in $deployDir"
}
